...
class PagesJaunesScraper(BaseScraper):
    ...
    async def _extract_lead_from_card(self, card) -> ScrapedLead | None:
        ...
        try:
            # Company name
            name_el = card.locator('.denomination-links h3, .bi-denomination a, .company_name')
            ...
            if await link_el.count() > 0:
                ...
                if href:
                    lead.source_url = f"https://www.pagesjaunes.fr{href}" if href.startswith("/") else href
            ...
            # Address
            addr_el = card.locator('.bi-address .bi-address-street, .address-container .street-address, .bi-adresse')
            if await addr_el.count() > 0:
                lead.address = (await addr_el.first.text_content() or "").strip()

            # City + postal code
            locality_el = card.locator('.bi-address .bi-address-city, .address-container .locality, .bi-localite')
            ...
            # Phone
            phone_el = card.locator('.bi-phone .coord-et-numero .tel, .clickable_phone_number, .bi-phone-number')
            ...
            # Website
            website_el = card.locator('a.pj-link--website, a[data-pjlabel="site_internet"], a.bi-website')
